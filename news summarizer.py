...
def summarize():
    ...
    sentiment.insert('1.0',f'Polarity:{analysis.polarity},sentiments:{"positive" if analysis.polarity > 0 else "negative" if analysis.polarity < 0 else "neutral"}')
    ...
    print(f'article.publish_date: {article.publish_date}')
...
summary = tk.Text(root,height = 4, width = 100)
...
sentiment = tk.Text(root,height = 6, width = 100)
...
utext = tk.Text(root,height = 2, width = 100)
utext.pack()
